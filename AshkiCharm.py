...
async def enter_product_flavors(message: types.Message, state: FSMContext):
    ...
    with Session() as session:
        # Проверяем, есть ли уже созданный товар
        product = session.query(Product).filter_by(name=data['name']).first()

        # Если товара нет (значит он был удален ранее), создаем его заново
        if not product:
            product = Product(
                name=data['name'],
        ...
        # Если есть ошибки, запрашиваем повторный ввод вкусов
        if errors:
            await message.answer("⚠️ Ошибки в строках:\n" + "\n".join(errors[:5]) +
                                 "\n\n🔄 Введите вкусы и количества снова (каждый с новой строки):")
            return  # НЕ сбрасываем состояние, ждем исправленный ввод

        # Если вкусы корректны, добавляем их в базу
        if flavors:
